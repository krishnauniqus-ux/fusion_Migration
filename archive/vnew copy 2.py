...
def clean_column_names(columns, header_row=None):
    ...
    Removes generic Column_1, Column_2, Unnamed: placeholders.
    """
    cleaned = []
    for i, col in enumerate(columns):
        col_str = str(col).strip()
        
        # Remove generic placeholder names - these should be replaced with actual headers
        if (col_str.startswith('Unnamed:') or 
            col_str.startswith('Column_') or
            col_str.lower() in ['nan', 'none', ''] or 
            pd.isna(col) or
            col_str == 'nan'):
            # When header_row is specified, we expect real values
            # Don't create fake Column_N names, let the user know data needs proper headers
    ...
    try:
        ...
        if file_type == 'csv':
            ...
            for sheet in sheets_to_process:
                try:
                    # First, read without headers to get the raw data preview
                    raw_df = pd.read_excel(io.BytesIO(file_bytes), sheet_name=sheet, header=None, nrows=50)
                    ...
                    # Only drop columns if there are data rows, otherwise keep header columns
                    if len(df) > 0:
    ...
    if uploaded:
        ...
        if st.session_state.template_file.selected_sheet:
            with st.expander("🔧 Debug Info", expanded=False):
                ...
                st.write(f"DataFrame columns: {list(df_debug.columns)[:10]}...")  # Show first 10
        ...
        if st.session_state.template_file.selected_sheet:
            ...
            if len(df.columns) > 0:
                ...
                if generic_columns:
                    st.warning(f"⚠️ Found {len(generic_columns)} generic column names. Try selecting a different header row.")
        ...
        if st.session_state.transformed_data is not None:
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    file_name=f"mapped_data_{st.session_state.session_id}.xlsx",
