...
def render_mapping_tab():
    ...
    # Left-Right Mapping Interface with enhanced styling
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(6, 182, 212, 0.1) 100%); 
                border-radius: 12px; padding: 2rem; margin: 1.5rem 0; border: 1px solid rgba(59, 130, 246, 0.2);">
        <div style="display: flex; align-items: center; margin-bottom: 1.5rem;">
            <div style="flex: 1; text-align: center;">
                <div style="background: rgba(59, 130, 246, 0.15); border: 2px solid #3b82f6; border-radius: 8px; 
                            padding: 0.75rem; font-weight: 700; color: #1e40af; font-size: 1.1rem;">
                    📊 SOURCE COLUMNS
                </div>
                <div style="color: #64748b; font-size: 0.9rem; margin-top: 0.5rem;">
                    {len(source_cols)} available columns
                </div>
            </div>
            <div style="width: 60px; text-align: center; margin: 0 1rem;">
                <div style="background: linear-gradient(135deg, #3b82f6 0%, #06b6d4 100%); 
                            border-radius: 50%; width: 50px; height: 50px; display: inline-flex; 
                            align-items: center; justify-content: center; color: white; 
                            font-weight: 700; font-size: 1.5rem; box-shadow: 0 4px 15px rgba(59, 130, 246, 0.4);">
                    →
                </div>
            </div>
            <div style="flex: 1; text-align: center;">
                <div style="background: rgba(16, 185, 129, 0.15); border: 2px solid #10b981; border-radius: 8px; 
                            padding: 0.75rem; font-weight: 700; color: #047857; font-size: 1.1rem;">
                    🎯 TARGET COLUMNS
                </div>
                <div style="color: #64748b; font-size: 0.9rem; margin-top: 0.5rem;">
                    {len(template_cols)} required columns
                </div>
            </div>
        </div>
    </div>
    ...
    # Modern Card-Based Mapping Interface using pure Streamlit components
    for i, target in enumerate(filtered_template_cols):
        ...
        # Create card container using Streamlit components only
        if is_mapped:
            # Green success card for mapped columns
            with st.success(f"✅ {target} - Successfully Mapped"):
                pass
        else:
            # Blue info card for unmapped columns  
            with st.info(f"⏳ {target} - Ready to Map"):
                pass
        
        # Create sub-container for the mapping controls
        with st.container():
            # Header row with target column info and badges
            col_header_left, col_header_right = st.columns([3, 1])
            
            with col_header_left:
                # Status is already shown in the card above, just show column name
            ...
            selected = st.selectbox(
                f"Map '{target}' from source column:",
                options,
                index=current_index,
                key=f"map_{target}_{i}",
                help=f"Select which source column should populate '{target}'",
            ...
            if selected_clean:
                ...
                if confidence_score > 0:
                    ...
                    st.progress(confidence_score, text=f"{confidence_pct}% match confidence")
            
            # Show validation rules preview
            if has_rules:
                active_rules = rule.get_active_rules()[:3]  # Show first 3 rules
                rules_preview = ", ".join(active_rules)
                if len(rule.get_active_rules()) > 3:
                    rules_preview += f" +{len(rule.get_active_rules()) - 3} more"
                
                st.caption(f"📋 Validation rules: {rules_preview}")
