...
def render_mapping_tab():
    ...
    if not st.session_state.template_file.selected_sheet or not st.session_state.source_file.selected_sheet:
        st.warning("Warning Please upload both Template and Data Source files first")
        return
    ...
    with col1:
        if st.button("Bot Auto-Map Columns", use_container_width=True):
            with st.spinner("Analyzing column similarities..."):
                ...
                st.success(f"Success Created {len(auto_mappings)} automatic mappings")
                st.rerun()
    ...
    if unmapped_mandatory:
        st.markdown(
            ...
            f'<strong>Warning Warning:</strong> Mandatory columns not mapped: '
            f'{", ".join(unmapped_mandatory)}</div>',
            unsafe_allow_html=True
        )
    
    # Show current mappings summary
    with st.expander("Clipboard View Current Mappings", expanded=False):
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Left Back to Rules", use_container_width=True):
            st.session_state.active_tab = 2
            st.rerun()
    with col2:
